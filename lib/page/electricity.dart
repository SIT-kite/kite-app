...
Widget _buildTextInputBox(hintText1, callBack1, hintText2, callBack2) {
  return SizedBox(
    width: 300,
    height: 60,
    child: Row(children: [_buildTextInput(hintText1, 2, callBack1), _buildTextInput(hintText2, 4, callBack2)]),
  );
}
      ...
      child: Column(
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text('0.00', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 32, color: Colors.black)),
            Text('元', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.grey)),
          ]),
          Text('24小时消费超越了 ${0.00}% 的寝室', style: TextStyle(fontSize: 16)),
          Container(margin: EdgeInsets.only(top: 5, bottom: 5), height: 1, color: Colors.blue),
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          TextButton(
            ...
            child: Row(children: [
              Text(
                '过去一天',
                style: TextStyle(fontSize: 20, color: showDays ? Colors.grey : Colors.blue),
              ),
              const Text(
                ' / ',
              ...
              Text(
                '过去一周',
                style: TextStyle(fontSize: 20, color: showDays ? Colors.blue : Colors.grey),
              )
            ]),
          ),
        ]),
...
void _initHoursAxisYData(List<Map<String, dynamic>> hoursData, List<FlSpot> hoursAxisYData) {
  for (int i = 0; i < hoursData.length; i++) {
    hoursAxisYData.add((FlSpot(i.toDouble(), hoursData[i]['consumption'].toDouble())));
  }
}
...
String _getCharge(List<Map<String, dynamic>> data) {
  double charge = 0.0;
  data.forEach((item) {
    charge = item['consumption'].toDouble() > charge ? item['consumption'].toDouble() : charge;
  });
