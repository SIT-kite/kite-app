...
class BillPage extends StatelessWidget {
  ...
  List<ExpenseRecord> expenseData =
      StoragePool.expenseRecordStorage.getAllByTimeDesc().length == 0
          ? []
          : StoragePool.expenseRecordStorage.getAllByTimeDesc();
  Future<void> _onBillRefresh(BuildContext context) async {
    await ExpenseRemoteService(SessionPool.ssoSession).getExpensePage(true, 1);
    expenseData = StoragePool.expenseRecordStorage.getAllByTimeDesc();
    _refreshController.refreshCompleted(resetFooterState: true);
  }

  Future<List<ExpenseRecord>> _expenseBills() async {
    ExpensePage firstPage = await ExpenseRemoteService(SessionPool.ssoSession)
        .getExpensePage(false, 1);
  ...
  Widget _buildBody(BuildContext context, List<ExpenseRecord> expenseData) {
    return SmartRefresher(
      ...
      onRefresh: () => _onBillRefresh(context),
      child: GroupedListView<ExpenseRecord, int>(
  ...
  Widget build(BuildContext context) {
    return StoragePool.expenseRecordStorage.getAllByTimeDesc().length == 0
